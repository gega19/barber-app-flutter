...
class SummaryStep extends StatelessWidget {
                  ...
                  BlocBuilder<PaymentMethodCubit, PaymentMethodState>(
                    buildWhen: (previous, current) {
                      ...
                      if (previous.runtimeType != current.runtimeType)
                        return true;
                      if (previous is PaymentMethodLoaded &&
                          current is PaymentMethodLoaded) {
                        // Rebuild si cambió el método seleccionado o la lista de métodos
                        if (previous.paymentMethods.length !=
                            current.paymentMethods.length)
                          return true;
                        if (selectedPaymentId != null) {
                          try {
                            final prevMethod = previous.paymentMethods
                                .firstWhere((m) => m.id == selectedPaymentId);
                            final currMethod = current.paymentMethods
                                .firstWhere((m) => m.id == selectedPaymentId);
  ...
  Widget _buildSummaryRowWithSubtitle(
    String label,
    String value,
    String subtitle,
    IconData icon,
